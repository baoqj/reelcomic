...
export const Details: React.FC = () => {
    ...
    const statusLabel = translateStatus(drama.status);
            ...
            <div className="relative w-full h-[50vh] md:h-[60vh] lg:h-[70vh]">
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-10 lg:p-20 max-w-[1440px] mx-auto">
                     <div className="flex items-center gap-3 mb-4">
                        <span className="px-2 py-1 bg-primary text-white text-[10px] font-bold rounded tracking-wider uppercase">{t('details.hotSeries')}</span>
                        <div className="flex items-center gap-1 text-yellow-400 text-sm font-bold">
                            <Icon name="star" filled className="text-base" /> {drama.rating}
                        </div>
                        <span className="text-gray-500 dark:text-white/60 text-sm font-medium">{drama.views} {t('details.views')}</span>
                    </div>
            ...
            <div className="max-w-[1440px] mx-auto px-6 lg:px-20 mt-12 flex flex-col lg:flex-row gap-12">
                ...
                <div className="lg:w-1/3 space-y-8">
                     <div className="bg-white dark:bg-white/5 p-6 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{t('details.aboutDrama')}</h3>
                        <div className="space-y-4">
                            <div>
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{t('details.genres')}</p>
                                <div className="flex flex-wrap gap-2">
                                    {drama.tags.map(g => (
                                        <span key={g} className="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full">{translateGenre(g)}</span>
                                    ))}
                                </div>
                            </div>
                            <div>
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{t('details.status')}</p>
                                    <p className="text-sm font-bold text-gray-700 dark:text-gray-200 flex items-center gap-2">
                                    <span className="size-2 bg-green-500 rounded-full animate-pulse"></span>
                                    {statusLabel} ({drama.episodes.length} / {drama.totalEpisodes} {t('details.episodes')})
                                </p>
                            </div>
                             <div>
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{t('details.cast')}</p>
                                <div className="flex flex-col gap-3">
                                    ...
                                    )) : <p className="text-sm text-gray-500">{t('details.castUnavailable')}</p>}
                                </div>
                            </div>
                        </div>
                     </div>
                ...
                <div className="flex-1">
                     <div className="mb-10">
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{t('details.synopsis')}</h3>
                     ...
                     <div className="flex items-center justify-between mb-6">
                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{t('details.episodes')}</h3>
                     ...
                     <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {drama.episodes.length > 0 ? drama.episodes.map((ep) => (
                            ...
                            >
                                ...
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">{t('details.episodeNumber', { number: ep.number })}</p>
                                <p className="text-sm font-bold text-gray-800 dark:text-gray-200 truncate">{ep.title}</p>
                            </div>
                        )) : (
                            <div className="col-span-full text-center py-10 text-gray-500">{t('details.noEpisodes')}</div>
                        )}
                     </div>
                </div>
            </div>
